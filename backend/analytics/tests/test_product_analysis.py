...
from analytics.services.llm_analysis import LLMAnalysisError
...
class ProductAnalysisPersistenceTests(TestCase):
    ...
    def test_llm_error_falls_back_without_losing_deterministic_score(self):
        ...
        self.assertEqual(
            fallback.input_snapshot["explanation"]["llm_status"],
    ...
    def test_malformed_llm_response_uses_fallback(self):
        ...
        self.assertEqual(analysis.provider, "deterministic")
        self.assertEqual(
            analysis.input_snapshot["explanation"]["llm_status"],
            "fallback_after_error",
        )

    def test_valid_llm_response_is_persisted_with_source(self):
        analysis = create_product_analysis(
            self.product,
