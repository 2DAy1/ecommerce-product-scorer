...
def _finalize_trends_timeout(job: JobRun, progress: _BatchProgress) -> None:
    _finalize_batch_job(
        job,
        status=JobRun.Status.FAILED,
        progress=progress,
        error_message=TRENDS_SOFT_TIMEOUT_MESSAGE,
        details={
...
def _finalize_trends_rate_limit(
    job: JobRun,
    *,
    total_items: int,
    progress: _BatchProgress,
) -> None:
    previous_failed_items = progress.failed_items
    skipped_items = max(
        0,
        total_items - progress.processed_items - previous_failed_items,
    )
    progress.failed_items = previous_failed_items + skipped_items
    _finalize_batch_job(
        job,
        status=JobRun.Status.FAILED,
        progress=progress,
        error_message=GOOGLE_TRENDS_RATE_LIMIT_MESSAGE,
        details={
            "rate_limited": True,
...
def _record_trends_batch_failure(
    *,
    total_items: int,
    progress: _BatchProgress,
    exc: Exception,
    ...
    progress.failed_items += max(0, unprocessed_items)
    progress.errors.append(
...
def _finalize_trends_completion(
    job: JobRun,
    *,
    total_items: int,
    progress: _BatchProgress,
    ...
    if progress.processed_items:
        ...
        error_message = (
            f"{progress.failed_items} of {total_items} products failed trend collection"
            if progress.failed_items
            else ""
        )
    else:
        status = JobRun.Status.FAILED
        error_message = f"All {progress.failed_items} products failed trend collection"
...
def collect_google_trends(self, job_id: str) -> dict[str, int]:
    ...
    if not products:
        _finish_empty_job(
            job,
            "No products available for Google Trends collection",
        )
    ...
    try:
        with GoogleTrendsCollector(
            ...
            _collect_product_trends(
                products,
                collector=collector,
                progress=progress,
            )
    except SoftTimeLimitExceeded:
        _finalize_trends_timeout(job, progress)
        raise
    except GoogleTrendsRateLimitError:
        _finalize_trends_rate_limit(
            job,
            total_items=len(products),
            progress=progress,
        )
        ...
        _record_trends_batch_failure(
            total_items=len(products),
            progress=progress,
            exc=exc,
        )

    _finalize_trends_completion(
        job,
        total_items=len(products),
        progress=progress,
    )
